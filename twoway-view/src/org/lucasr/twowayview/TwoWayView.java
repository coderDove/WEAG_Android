...
public class TwoWayView extends AdapterView<ListAdapter> implements
    ...
    private int mDefaultChildWith;
    private int mItemOffset;
    private View mSelectItem;
  
    private ISelectItem mSelectListener;
    ...
    public void setSelectListener(ISelectItem listener) {
	...
	if (mSelectItem != null) {
		...
		mSelectListener
			.onSelectItem(getPositionForView(mSelectItem) - 2);
	    setSelectionInt(getPositionForView(mSelectItem) - 2);
	}
    }

    public void setSelectItem(View selectItem) {
	...
	case MotionEvent.ACTION_DOWN: {
		...
		return true;
	    } else if (mMotionPosition >= 0 && mAdapter != null
	...
	case MotionEvent.ACTION_UP: {
		...
		if (Math.abs(velocity) >= mFlingVelocity) {
		    ...
		    // TODO this code omitted for stop fast(computer) scrolling
    ...
    public int getDefaultChildWidth() {
	return mDefaultChildWith;
    }
	...
	return getScrollX() + getWidth() / 2 + mDefaultChildWith / 2;
    }
	...
	int mItemWidth = mDefaultChildWith;
	rightShadow.setBounds(getLeftBorder(), 0,
		getWidth() + getScrollX() + 1, getHeight());
	rightShadow.draw(canvas);

	centerShadow.setBounds(getScrollX() + getWidth() / 2 - mItemWidth / 2,
		0, getScrollX() + getWidth() / 2 + mItemWidth / 2, getHeight());
	centerShadow.draw(canvas);

	leftShadow.setBounds(getScrollX(), 0, getScrollX() + getWidth() / 2
		- mItemWidth / 2, getHeight());
	...
	if (widthSize % 5 != 0) {
	    mDefaultChildWith = (widthSize / 5) + 1;
	} else {
	    mDefaultChildWith = (widthSize / 5);
	}
