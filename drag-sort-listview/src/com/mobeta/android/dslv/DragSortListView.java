...
public class DragSortListView extends ListView {
	...
	private List<Boolean> mIsUndoList;
	...
	private void fillIsUndoList() {
	    mIsUndoList = new ArrayList<Boolean>(getCount());
	    for (int i = 0; i < getCount(); i++) {
		mIsUndoList.add(Boolean.valueOf(false));
	    }
	}

	public void setUndo(boolean isUndo, int position) {
	    if (mIsUndoList != null && position < mIsUndoList.size()) {
		mIsUndoList.set(position, Boolean.valueOf(isUndo));
	    }
	}

	public boolean isUndo(int position) {
	    if (mIsUndoList != null && position < mIsUndoList.size()) {
		boolean isUndo = mIsUndoList.get(position);
		return isUndo;
	    }
	    return false;
	}

	public void changeUndoItem(int position) {
	    setUndo(false, position);
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		View oldChildUndo = v.getChildAt(1);
		((UndoView) oldChildUndo).setPosition(position);

		child = mAdapter.getView(position, oldChild,
			DragSortListView.this);
		if (isUndo(position)) {
		    child.setVisibility(View.GONE);
		    oldChildUndo.setVisibility(View.VISIBLE);
		} else {
		    child.setVisibility(View.VISIBLE);
		    oldChildUndo.setVisibility(View.GONE);
		}
		if (child != oldChild) {
		...
		child.setVisibility(View.VISIBLE);
		v.addView(child);
		final UndoView undoView = new UndoView(getContext());
		undoView.setVisibility(View.GONE);
		undoView.setPosition(position);
		undoView.setOnUndoClickListener(new OnClickListener() {

		    @Override
		    public void onClick(View v) {
			setUndo(false, undoView.getPosition());
			notifyDataSetChanged();
		    }
		});
		v.addView(undoView);
	...
	if (mDragState == IDLE || mDragState == DRAGGING) {
		...
		final int position = mSrcPos - getFirstVisiblePosition();
		View v = getChildAt(mSrcPos - getFirstVisiblePosition());
		if (v != null && mAdapterWrapper != null) {
		    boolean isUndo = !mAdapterWrapper.isUndo(position);
		    // ((IUndoDragSortListView) v).showUndoView(isUndo);
		    if (!isUndo) {
			((IRemoveItemListener) getContext()).onRemove(position);
		    }
		    mAdapterWrapper.setUndo(isUndo, position);
		}
	    } else {
		doRemoveItem(which);
	    }
     ...
     *                  ~{@link #DRAG_NEG_X}</code> to allow dragging the
     *            floating View in all directions except off the screen to the
     *            left.
     * @param deltaX
	...
	// 3 - flag move / up down
	// if (mDragFlags == 3) {
	// if (srcItem instanceof IUndoDragSortListView) {
	// // ((IUndoDragSortListView) srcItem).showUndoView(true);
